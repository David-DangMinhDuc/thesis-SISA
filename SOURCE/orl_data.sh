...
if [[ ! -f general-report.csv ]]; then
    echo "nb_shards,nb_slices,nb_epochs,nb_requests,accuracy,retraining_time,nb_retrained_points" > results/report_"${shards}"_shards_"${slices}"_slices_"${epochs}"_epochs.csv
fi
...
for j in {0..15}; do
    ...
    echo "${shards},${slices},${epochs},${r},${acc},${time},${numOfRetrainPoints}" >> results/report_"${shards}"_shards_"${slices}"_slices_"${epochs}"_epochs.csv
done
