...
model_lib = import_module("models.{}".format(args.model))

# Retrive dataset metadata.
with open(args.dataset) as f:
    face_data_info = json.loads(f.read())
input_shape = tuple(face_data_info["input_shape"])
nb_classes = face_data_info["nb_classes"]
...
model = None
if "/orl/" in args.dataset:
    model = model_lib.orlModel(input_shape, nb_classes, dropout_rate=args.dropout_rate)
elif "/ar/" in args.dataset:
    model = model_lib.arModel(input_shape, nb_classes, dropout_rate=args.dropout_rate)
else:
    raise "Unsupported face dataset"
